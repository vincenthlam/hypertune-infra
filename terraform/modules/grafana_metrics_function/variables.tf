...
variable "rule_actions_actions_action_type" {
  description = "The type of action for the event rule. Options: FN (Functions), STREAMING (Streaming), NOTIFICATIONS (Notifications)."
  type        = string
  default     = "FAAS" # Default to Functions
}

variable "rule_actions_actions_is_enabled" {
  description = "Whether the event rule action is enabled. Set to true to enable the action, or false to disable."
  type        = bool
